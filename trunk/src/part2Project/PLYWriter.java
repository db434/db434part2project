package part2Project;

import java.io.*;

public class PLYWriter
{
	public static void writeFile(String fileName, HalfEdgeScheme hes) throws Exception
	{
		int vertices = hes.numVertices();
		int faces = hes.numFaces();
		
		BufferedWriter out = new BufferedWriter(new FileWriter(fileName));
		
		out.write(header(vertices, faces));
		
		for(int i=0; i<vertices; i++)
		{
			out.write(hes.getVertex(i).toString() + "\n");
		}
		
		for(int i=0; i<faces; i++)
		{
			out.write(hes.getFace(i).toString() + "\n");
		}
		
		out.close();
	}
	
	private static String header(int vertices, int faces)
	{
		return new String("" +
				"ply\n" +
				"format ascii 1.0\n" +
				"comment Generated by Daniel Bates's Part II Project\n" +
				"element vertex " + vertices + "\n" +
				"property float32 x\n" +
				"property float32 y\n" +
				"property float32 z\n" +
				"element face " + faces + "\n" +
				"property list uint8 int32 vertex_index\n" +
				"end_header\n");
	}
}
